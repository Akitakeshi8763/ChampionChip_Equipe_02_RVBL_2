module address_decoder #(
    ...
    // 1. Address Alignment: Enforce 4-byte alignment[cite: 1]
    wire [ADDR_W-1:0] aligned_address;
    assign aligned_address = {core_address_i[ADDR_W-1:2], 2'b00};

    // Both memory blocks receive the aligned address[cite: 1]
    assign imem_address_o = aligned_address;
    assign dmem_address_o = aligned_address;

    // Only DMEM receives the byte-write mask[cite: 1]
    ...
    always @(*) begin
        ...
        if ((core_address_i >= IMEM_BASE) && (core_address_i < (IMEM_BASE + IMEM_SIZE))) begin
            ...
            dmem_oe_o  = core_oe_i;[cite: 1]
            dmem_we_o  = core_we_i;[cite: 1]
            sel_o = 1'b0;
        end
    end
